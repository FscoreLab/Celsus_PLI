...
from .mlm import MLM
from .visual_ssl import SimCLR, SimSiam
        ...
        for _ in range(depth):
            self.layers.append(
                nn.ModuleList(
                    [
                        PreNorm(
                            dim, Attention(dim=dim, dim_head=dim_head, heads=heads, causal=causal, dropout=attn_dropout)
                        ),
...
class TextTransformer(nn.Module):
    def __init__(self, dim, *, num_tokens, max_seq_len, dim_head, rotary_pos_emb=None, causal=False, **kwargs):
        super().__init__()
...
class VisionTransformer(nn.Module):
    def __init__(self, dim, *, image_size, patch_size, channels, patch_dropout=0.5, **kwargs):
        super().__init__()
        ...
        self.to_cls_tokens = nn.Sequential(
            Reduce("b n d -> b d", "mean"), nn.Linear(dim, dim, bias=False), Rearrange("b d -> b 1 d")
        )
...
class CTCLIP(nn.Module):
    def __init__(
        ...
        **kwargs,
        ...
        if use_mlm:
            ...
            self.mlm = MLM(self.text_transformer, dim=dim_text, num_tokens=num_text_tokens, **mlm_kwargs)
        ...
        if self.use_visual_ssl:
            if exists(visual_ssl):
                ...
                self.visual_ssl = ssl_type(
                    self.visual_transformer, image_size=visual_image_size, hidden_layer=visual_ssl_hidden_layer
                )
        ...
        # Use cached tokenizer to avoid repeated downloads
        if not hasattr(CTCLIP, "_cached_tokenizer"):
            CTCLIP._cached_tokenizer = BertTokenizer.from_pretrained(
                "microsoft/BiomedVLP-CXR-BERT-specialized", do_lower_case=True
            )
        self.tokenizer = CTCLIP._cached_tokenizer
    ...
    def forward(
        ...
        if self.text_causal_mask:
            ...
            eos_text_mask = torch.zeros_like(eos_text_mask).scatter(1, eos_indices, 1.0).bool()
        ...
        if self.use_all_token_embeds:
            ...
            if self.extra_latent_projection:
                sim_image_to_text = (
                    einsum("m x t d, n y i d -> m n x y t i", text_latents_extra, image_latents_extra) * temp
                )
        ...
        if self.decoupled_contrastive_learning:
            ...
            text_to_image_exp, image_to_text_exp = map(
                lambda t: t.masked_fill(pos_mask, 0.0), (text_to_image_exp, image_to_text_exp)
            )
