...
from .utils import get_bbox_slices, speedup_numpy_unique_cpu
...
class NiftiInferenceSeparateMasks:
    ...
    def __init__(self, model_path: str, mae_weights_path: str, bert_path: str, config_path: str = None):
        ...
        self.image_loader = transforms.Compose(
            [
                *base_load_transforms,
                transforms.ScaleIntensityRanged(keys=["arr"], a_min=-1150, a_max=350, b_min=0.0, b_max=1.0, clip=True),
            ]
        )

        self.pad_func = transforms.DivisiblePadd(
            keys=["image", "label"], k=(16, 16, 32), mode="constant", constant_values=0, method="end"
        )
    ...
    def _load_model(self, model_path: str, mae_weights_path: str, bert_path: str, config_path: str = None):
        ...
        args = argparse.Namespace(
            cfg_path=config_path, options=[f"model.med_config_path={os.path.join(bert_path, 'config.json')}"]
        )
        cfg = Config(args)
    ...
    def preprocess_nifti_with_mask_array(
        self, image_path: str, mask_array: np.ndarray
        ...
        padder = transforms.Compose(
            [
                transforms.SpatialPadd(
                    keys=["image", "label"], spatial_size=(112, 256, 352), mode="constant", constant_values=0
                )
            ]
        )
    ...
    def predict_single(self, image_path: str, save_mask_path: str = None) -> Dict[str, float]:
        ...
        mask_array = transforms.Transposed(keys=["arr"], indices=(0, 3, 2, 1))({"arr": mask_array[None]})["arr"][
            0
        ].numpy()

        print(f"✓ Маска готова, размер: {mask_array.shape}")

        # Проверяем наличие лёгких в маске (ID=1 для лёгких)
        lung_mask = mask_array == 1
        if not lung_mask.any():
            print("❌ Лёгкие не найдены в маске (все значения 0)")
            from .lct_validation import LungsNotFoundError

            raise LungsNotFoundError()

        validator = LungValidator()
        # Сначала проверяем наличие лёгких (occupancy ratio)
        validator.validate_chest(lung_mask, spacing[-1])  # TODO: check spacing order
        # Затем проверяем длину лёгких
        validator.validate_lung_length(lung_mask, spacing[-1])  # TODO: check spacing order
        ...
        for organ_name, organ_tasks in organ_test_items.items():
            ...
            window_patch = torch.nn.functional.interpolate(
                input=torch.tensor(window_patch[None]),
                size=roi_size,
                mode="trilinear",
            )
            window_mask = torch.nn.functional.interpolate(
                input=torch.tensor(window_mask[None]),
                size=roi_size,
                mode="nearest",
            )
...
def print_results_summary(results: Dict[str, float], filename: str, test_items: List):
    ...
    heart_results = {
        k: v for k, v in results.items() if any(item[0] == "heart" and item[1] == k for item in test_items)
    }
    esophagus_results = {
        k: v for k, v in results.items() if any(item[0] == "esophagus" and item[1] == k for item in test_items)
    }
    aorta_results = {
        k: v for k, v in results.items() if any(item[0] == "aorta" and item[1] == k for item in test_items)
    }
...
def main():
    ...
    parser.add_argument("--model_path", type=str, required=True, help="Путь к файлу модели (.pth)")
    parser.add_argument("--mae_weights_path", type=str, required=True, help="Путь к предобученным MAE весам")
    parser.add_argument(
        "--bert_path", type=str, required=True, help="Путь к директории с BiomedVLP-CXR-BERT-specialized"
    )
    parser.add_argument("--config_path", type=str, default=None, help="Путь к конфигурационному файлу")
    ...
    mask_group.add_argument("--separate_masks", action="store_true", help="Использовать отдельные маски органов")
    mask_group.add_argument(
        "--use_totalsegmentator", action="store_true", help="Использовать TotalSegmentator для генерации масок"
    )

    # Опции для сохранения
    parser.add_argument("--save_mask_path", type=str, help="Путь для сохранения сгенерированной маски Totalsegmentator")
    parser.add_argument(
        "--output_json",
        type=str,
    ...
    parser.add_argument("--verbose", action="store_true", help="Подробный вывод результатов для каждого файла")
    ...
    for i, image_path in enumerate(tqdm(nifti_files, desc="Обработка файлов")):
        ...
        try:
            results = inference.predict_single(image_path, save_mask_path=args.save_mask_path)
    ...
    if all_results:
        ...
        if all_results:
            ...
            lung_pathologies = [
                ...
                if any(item[0] == "lung" and item[1] == k for item in inference.test_items)
            ]
            heart_pathologies = [
                k
                for k in sample_result.keys()
                if any(item[0] == "heart" and item[1] == k for item in inference.test_items)
            ]
            esophagus_pathologies = [
                k
                for k in sample_result.keys()
                if any(item[0] == "esophagus" and item[1] == k for item in inference.test_items)
            ]
            aorta_pathologies = [
                k
                for k in sample_result.keys()
                if any(item[0] == "aorta" and item[1] == k for item in inference.test_items)
            ]
